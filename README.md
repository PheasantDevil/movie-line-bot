...
- ✅ **週次通知**: 過去1週間と先1週間の映画情報を毎日自動通知
- ✅ **上映館数表示**: 限定公開映画を⚠️マークで表示
- ✅ **映画検索**: LINEで映画名を入力すると情報を返信
- ✅ **LINE Messaging API**: Push通知とWebhook対応
...
- Webhookサーバー: 無料枠あり（Render.com推奨）
- **総コスト: ¥0/月**（無料プランの場合）

## 🔧 Webhook機能の設定

キーワード検索機能を有効にするには、Webhookサーバーのデプロイが必要です。

**→ [Webhookセットアップガイド](WEBHOOK_SETUP.md)** ← 詳細はこちら
